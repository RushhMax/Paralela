...
int main(int argc, char* argv[]){
    ...
    if (rank == 0) {
        ...
        if (n % q != 0) {
            printf("Error: n debe ser divisible por q=%d\n", q);
            MPI_Finalize();
            return 1;
        }
    ...
    // Distribuir bloques de la matriz
    if (rank == 0) {
    ...
    // Distribuir bloques del vector
    if (rank == 0) {
        for (int i = 0; i < block_size; i++)
            V_local[i] = V[row*block_size + i];
    }
    MPI_Bcast(V_local, block_size, MPI_DOUBLE, row *q, grid_comm);
